...
import { deleteUserByIdQuery } from "../../../../../../query/User";
import ChangeUserRoleForm from "../../form/ChangeUserRoleForm";
import ChangeUserLoginForm from "../../form/ChangeUserLoginForm";
import {
  getJwtTokenLocaleStorageParam,
  getUserLoginLocaleStorageParam,
...
const UserItem = (props) => {
  ...
  const user = props.valueUser;
  const userLogin = getUserLoginLocaleStorageParam();

  const [responceException, setResponceException] = useState("");
  const [isShowChangeUserBtns, setIsShowChangeUserBtns] = useState(false);
  const [isShowModalChangeUserRoleForm, setIsShowModalChangeUserRoleForm] =
    useState(false);
  const [isShowModalChangeUserLoginForm, setIsShowModalChangeUserLoginForm] =
    useState(false);
  const [isShowConfirmDeleteUserBtns, setIsShowConfirmDeleteUserBtns] =
    useState(false);
  ...
  const deleteUser = async () => {
    ...
    validationJwtTokenAdmin(token)
      .then(async (responce) => {
        if (responce.ok) {
          deleteUserByIdQuery(token, user.id)
            .then(async (response) => {
              if (response.ok) {
                setIsShowConfirmDeleteUserBtns(false);
                props.onChangeUser();
  ...
  const changeUserRoleFormModal = (
    <Modal
      size="xl"
      show={isShowModalChangeUserRoleForm}
      backdrop="static"
      onHide={() => {
        setResponceException("");
        setIsShowModalChangeUserRoleForm(false);
        setIsShowChangeUserBtns(false);
      }}
    ...
    >
      <Modal.Header closeButton>
        ...
        >
          <div className="d-flex justify-content-between align-items-center">
            <h4>
              {(locale === LOCALE_RU && `Изменение роли пользоателя`) ||
                (locale === LOCALE_EN && `Change role of user`)}
            </h4>
          </div>
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <ChangeUserRoleForm
          ...
          valueUserLogin={user.login}
          valueUserRoleId={user.role.id}
          onChangeUser={() => {
            setIsShowModalChangeUserRoleForm(false);
            setIsShowChangeUserBtns(false);
            props.onChangeUser();
  ...
  const changeUserLoginFormModal = (
    <Modal
      size="xl"
      show={isShowModalChangeUserLoginForm}
      backdrop="static"
      onHide={() => {
        setResponceException("");
        setIsShowModalChangeUserLoginForm(false);
        setIsShowChangeUserBtns(false);
      }}
      dialogClassName="modal-90w"
      aria-labelledby="example-custom-modal-styling-title"
    >
      <Modal.Header closeButton>
        ...
        >
          <div className="d-flex justify-content-between align-items-center">
            <h4>
              {(locale === LOCALE_RU && `Изменение логин пользоателя`) ||
  ...
  return (
    <tr style={{ height: "46px" }}>
      ...
      <td className="text-center">
        {user.login}
        {isShowChangeUserBtns && user.login !== userLogin && (
          <div>
            <button
              className="bg-transparent w-100"
              style={{ border: "none", margin: "0 auto" }}
              variant="primary"
              onClick={() => setIsShowModalChangeUserLoginForm(true)}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                x="0px"
            ...
            {isShowModalChangeUserLoginForm && changeUserLoginFormModal}
          </div>
        )}
      </td>
      <td className="text-center">
        ...
        {isShowChangeUserBtns && user.login !== userLogin && (
          <div>
            <button
              ...
              style={{ border: "none", margin: "0 auto" }}
              variant="primary"
              onClick={() => setIsShowModalChangeUserRoleForm(true)}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                x="0px"
            ...
            {isShowModalChangeUserRoleForm && changeUserRoleFormModal}
          </div>
        )}
      </td>
      {userRole === ROLE_ADMIN && (
        <td>
          {user.login !== userLogin && (
            <button
              ...
              style={{ border: "none", margin: "0 auto" }}
              variant="primary"
              onClick={() => setIsShowChangeUserBtns(!isShowChangeUserBtns)}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                x="0px"
          ...
          {user.login !== userLogin && (
            <div>
              <button
                ...
                onClick={() => {
                  setResponceException("");
                  setIsShowConfirmDeleteUserBtns(!isShowConfirmDeleteUserBtns);
                }}
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
              ...
              {isShowConfirmDeleteUserBtns && confirmDeleteUserBtnsEl}
            </div>
          )}
        </td>
      )}
