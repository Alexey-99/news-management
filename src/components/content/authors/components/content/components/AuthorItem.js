...
import { deleteNewsByAuthorIdQuery } from "../../../../../../query/News";
import { LOCALE_EN, LOCALE_RU } from "../../../../../../locate/Locale";

const AuthorItem = (props) => {
  const userRole = props.valueUserRole;
  const locale = props.valueLocale;
  const author = props.valueAuthor;
  const [authorName, setAuthorName] = useState(author.name);
  const [responceException, setResponceException] = useState("");
  const [isShowModalUpdateAuthorForm, setIsShowModalUpdateAuthorForm] =
    useState(false);
  const [isShowConfirmDeleteAuthorBtns, setIsShowConfirmDeleteAuthorBtns] =
    useState(false);
  const [
    isShowConfirmDeleteNewsByAuthorBtns,
    setIsShowConfirmDeleteNewsByAuthorBtns,
  ] = useState(false);
      ...
      .then(async (responce) => {
        if (responce.ok) {
          updateAuthorQuery(token, {
            ...
            .then(async (response) => {
              if (response.ok) {
                setIsShowModalUpdateAuthorForm(false);
                props.onChangeAuthor();
  ...
  const deleteAuthor = async () => {
    ...
    validationJwtTokenAdmin(token)
      .then(async (responce) => {
        if (responce.ok) {
          deleteAuthorByIdQuery(token, author.id)
            .then(async (response) => {
              if (response.ok) {
                setIsShowConfirmDeleteAuthorBtns(false);
                props.onChangeAuthor();
                setResponceException("");
              } else if (response.status === 401) {
                const responseJson = await response.json();
  ...
  const deleteNewsByAuthorId = async () => {
    const token = getJwtTokenLocaleStorageParam();
    validationJwtTokenAdmin(token)
      .then(async (responce) => {
        if (responce.ok) {
          deleteNewsByAuthorIdQuery(token, author.id)
            .then(async (response) => {
              if (response.ok) {
                setIsShowConfirmDeleteNewsByAuthorBtns(false);
                props.onChangeAuthor();
                setResponceException("");
              } else if (response.status === 401) {
                const responseJson = await response.json();
  ...
  const updateAuthorFormModal = (
    <Modal
      size="xl"
      show={isShowModalUpdateAuthorForm}
      backdrop="static"
      onHide={() => {
        setResponceException("");
        setIsShowModalUpdateAuthorForm(false);
      ...
      <Modal.Header closeButton>
        ...
        >
          <div className="d-flex justify-content-between align-items-center">
            <h4>
              {(locale === LOCALE_RU && `Изменение автора`) ||
                (locale === LOCALE_EN && `Change of author`)}
            </h4>
          </div>
        </Modal.Title>
      </Modal.Header>
  ...
  const confirmDeleteAuthorBtnsEl = (
    <div>
      <div>
        {responceException !== "" && (
          <div
  ...
  const confirmDeleteNewsByAuthorIdBtnsEl = (
    <div>
      <div>
        {responceException !== "" && (
          <div
    ...
    <tr>
      ...
      {userRole === ROLE_ADMIN && (
        <td>
          <button
            ...
            variant="primary"
            onClick={() => setIsShowModalUpdateAuthorForm(true)}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              x="0px"
      ...
      {userRole === ROLE_ADMIN && (
        <td>
          ...
          {isShowConfirmDeleteNewsByAuthorBtns &&
            confirmDeleteNewsByAuthorIdBtnsEl}
        </td>
      )}
      {userRole === ROLE_ADMIN && (
        <td>
          {author.countNews === 0 && (
            <button
              ...
              onClick={() => {
                setResponceException("");
                setIsShowConfirmDeleteAuthorBtns(
                  !isShowConfirmDeleteAuthorBtns
                );
              }}
            >
              <svg
          ...
          {isShowConfirmDeleteAuthorBtns && confirmDeleteAuthorBtnsEl}
        </td>
      )}
    </tr>
  );
