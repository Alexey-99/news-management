...
const ChangeNewsForm = (props) => {
  ...
  const [titleNews, setTitleNews] = useState(props.valueNews.title);
  const [contentNews, setContentNews] = useState(props.valueNews.content);
  const [authorLoginNews, setAuthorLoginNews] = useState(
    props.valueNews.author.name
  );
            ...
            .then(async (response) => {
              if (response.ok) {
                ...
                if (locale === LOCALE_RU) {
                  ...
                  setResponceException("Something has gone wrong");
                }
              }
            })
            .catch(async (error) => {
